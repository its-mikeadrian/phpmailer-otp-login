...
class DbSessionHandler implements SessionHandlerInterface
{
    ...
    public function __construct(PDO $pdo)
    {
        ...
        $this->lifetime = (int) env_get('SESSION_LIFETIME_SECONDS', '1800');
    ...
    public function write($id, $data): bool
    {
        try {
            ...
            $stmt = $this->pdo->prepare('INSERT INTO `sessions` (`id`, `data`, `expires_at`, `last_activity`, `ip_address`, `user_agent`) VALUES (?, ?, ?, ?, ?, ?) 
                ON DUPLICATE KEY UPDATE `data` = VALUES(`data`), `expires_at` = VALUES(`expires_at`), `last_activity` = VALUES(`last_activity`), `ip_address` = VALUES(`ip_address`), `user_agent` = VALUES(`user_agent`)');
            return $stmt->execute([$id, $payload, $expires, $now, $ip, $ua]);
        } catch (Throwable $e) {
            return false;
        }
    }
